...
bool
WifiPhyStateHelper::IsStateFd (void)
{
  return (GetState () == WifiPhy::FD);
}
...
Time
WifiPhyStateHelper::GetLastTxEndTime (void) const
{
  return m_endTx;
}
...
{
  if (m_endTx > Simulator::Now() && m_rxing)
    {
      return WifiPhy::FD;
    }
...
void
WifiPhyStateHelper::NotifyTxPostpone (Time endTime)
{
  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
    {
      (*i)->NotifyTxPostpone (endTime);
    }
}
void
WifiPhyStateHelper::NotifyRxPostpone (Time endTime)
{
  for (Listeners::const_iterator i = m_listeners.begin (); i != m_listeners.end (); i++)
    {
      (*i)->NotifyRxPostpone (endTime);
    }
}
void
                                ...
                                WifiPreamble preamble, uint8_t txPower)
{
  ...
  switch (GetState ())
    ...
    case WifiPhy::RX:
      ...
      m_stateLogger (now, txDuration, WifiPhy::FD);
      break;
    case WifiPhy::CCA_BUSY:
      {
  ...
  m_startTx = now;
  NS_LOG_DEBUG("tx: start=" << m_startTx << " end=" << m_endTx);
}
void
WifiPhyStateHelper::PostponeTx(Time txEndTime)
{
  NS_LOG_FUNCTION(this <<" txEndTime=" << txEndTime);
  m_endTx = txEndTime;
  NotifyTxPostpone (txEndTime);
}

void
WifiPhyStateHelper::PostponeRx(Time rxEndTime)
{
  NS_LOG_FUNCTION(this << " rxEndTime=" << rxEndTime);
  m_endRx = rxEndTime;
  NotifyRxPostpone (rxEndTime);
}
  
void
WifiPhyStateHelper::SwitchToRx (Time rxDuration)
{
  NS_ASSERT (IsStateIdle () || IsStateCcaBusy () || IsStateTx () ||
             IsStateRx () || IsStateFd ());
  // NS_ASSERT (!m_rxing);
  ...
  switch (GetState ())
    ...
    case WifiPhy::SWITCHING:
      break;
    case WifiPhy::TX:
        m_stateLogger (m_startTx, now - m_startTx, WifiPhy::FD);
      break;
    case WifiPhy::RX:
    case WifiPhy::FD:
      // NS_FATAL_ERROR ("Invalid WifiPhy state.");
  ...
  NS_LOG_DEBUG("rx: start=" << m_startRx << " end=" << m_endRx);
}
