...
public:
  ...
  {
    m_dcf->NotifyTxPostponeNow (endTime);
  }
  virtual void NotifyRxPostpone (Time endTime)
  {
    m_dcf->NotifyRxPostponeNow (endTime);
  }
...
Time
DcfManager::MostRecent (Time a, Time b, Time c, Time d, Time e) const
{
  NS_LOG_FUNCTION (this << a << b << c << d << e);
  Time f = Max (a, b);
  Time g = Max (c, d);
  Time h = Max (e, f);
  Time retval = Max (g, h);
...
DcfManager::DoGrantAccess (void)
{
  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); k++)
    {
      DcfState *state = *i;
      NS_LOG_DEBUG ("backoffendfor=" << GetBackoffEndFor (state) << " nowtime=" << Simulator::Now ());
      if(state->IsAccessRequested ())
        {
          NS_LOG_DEBUG ("accessrequested = true");
        }
      else
        {
          NS_LOG_DEBUG ("accessrequested = false");
        }
      if (state->IsAccessRequested ()
...
DcfManager::GetAccessGrantStart (void) const
{
  ...
  NS_LOG_DEBUG("m_lastTxStart " << m_lastTxStart << "duration"<< m_lastTxDuration);
...
Time
DcfManager::GetAccessGrantStartForSecondary (void) const
{
  NS_LOG_FUNCTION (this);
  ...
  Time ctsTimeoutAccessStart = m_lastCtsTimeoutEnd + m_sifs;
  Time switchingAccessStart = m_lastSwitchingStart + m_lastSwitchingDuration + m_sifs;
  Time accessGrantedStart = MostRecent (txAccessStart,
                                        navAccessStart,
                                        ackTimeoutAccessStart,
                                        ctsTimeoutAccessStart,
                                        switchingAccessStart
...
Time
DcfManager::GetSecondaryGrantedTime ()
{
  for (States::iterator i = m_states.begin (); i != m_states.end (); i++)
    {
      DcfState *state = *i;
      Time accessGrantTime = GetAccessGrantStartForSecondary () + MicroSeconds (state->GetAifsn () * m_slotTimeUs);
      //      return accessGrantTime + MicroSeconds (state->GetBackoffSlots () * m_slotTimeUs);
      return accessGrantTime;
    }
  return Seconds(0);
}
          ...
          if (tmp > Simulator::Now ())
            {
              ...
              NS_LOG_DEBUG("accessTimeoutNeeded = true " << expectedBackoffEnd);
            }
        }
    }
  if (accessTimeoutNeeded)
    {
      ...
      if (m_accessTimeout.IsRunning ()
        ...
        {
          NS_LOG_DEBUG("cancel timeout");
          m_accessTimeout.Cancel ();
        }
      if (m_accessTimeout.IsExpired ())
        {
          NS_LOG_DEBUG("next timeout=" << expectedBackoffDelay + Simulator::Now ());
...
void
DcfManager::CancelAccessRequested ()
{
  for (States::iterator i = m_states.begin (); i != m_states.end (); i++)
    {
      DcfState *state = *i;
...
bool
DcfManager::IsBusyForFullDuplex ()
{
  NS_LOG_FUNCTION (this);
  // PHY busy
  Time lastTxEnd = m_lastTxStart + m_lastTxDuration;
...
void
DcfManager::NotifyTxPostponeNow (Time endTime)
{
  NS_LOG_FUNCTION (this << endTime);
  UpdateBackoff ();
  if (endTime > m_lastTxStart + m_lastTxDuration){
    m_lastTxDuration = endTime - m_lastTxStart;
  }
}
void
DcfManager::NotifyRxPostponeNow (Time endTime)
{
  NS_LOG_FUNCTION (this << endTime);
  UpdateBackoff ();
  if (endTime > m_lastRxStart + m_lastRxDuration){
    m_lastRxDuration = endTime - m_lastRxStart;
  }
}
void
