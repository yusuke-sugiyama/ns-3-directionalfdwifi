...
WifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiPhy")
    ...
    .AddTraceSource ("PhyRxHeaderEnd",
                     "Trace source indicating a header has been received",
                     MakeTraceSourceAccessor (&WifiPhy::m_phyRxHeaderEndTrace))
  ;
  return tid;
}
...
void
WifiPhy::SetMac (Ptr<MacLow> macLow)
{
  NS_LOG_FUNCTION(this);
  m_macLow = macLow;
}
...
void
WifiPhy::SetReceivingTxVector (WifiTxVector txVector)
{
  m_receivingTxVector = txVector;
}

void
WifiPhy::SetReceivingPreamble (WifiPreamble preamble)
{
  m_receivingPreamble = preamble;
}

void
WifiPhy::SetReceivingAddress4 (Mac48Address addr4)
{
  m_receivingAddress4 = addr4;
}
  
WifiTxVector
WifiPhy::GetReceivingTxVector ()
{
  return m_receivingTxVector;
}

WifiPreamble
WifiPhy::GetReceivingPreamble ()
{
  return m_receivingPreamble;
}

Mac48Address
WifiPhy::GetReceivingAddress4 ()
{
  return m_receivingAddress4;
}
...
uint32_t
WifiPhy::GetPacketSizeFromDuration (Time time, WifiTxVector txvector)
{
  ...
  // êÿÇËè„Ç∞
  uint32_t numSymbols = (time.GetDouble() / 1000)/ symbolDurationUs;
  double Nes=1;
  uint32_t numBytes = (((numSymbols * numDataBitsPerSymbol) - 16 - (6 * Nes)) / 8);
  return numBytes;
}
    ...
    case WIFI_MOD_CLASS_ERP_OFDM:
      {
        ...
        NS_LOG_DEBUG("DurationUs=" << symbolDurationUs << " Rate=" << payloadMode.GetDataRate () <<
                     " BitPerSymbol=" << numDataBitsPerSymbol << " numSymbols=" << numSymbols);
...
{
  WifiMode payloadMode=txvector.GetMode();
  double duration1 = GetPlcpPreambleDurationMicroSeconds (payloadMode, preamble);
  double duration2 = GetPlcpHeaderDurationMicroSeconds (payloadMode, preamble);
  double duration3 = GetPlcpHtSigHeaderDurationMicroSeconds (payloadMode, preamble);
  double duration4 = GetPlcpHtTrainingSymbolDurationMicroSeconds (payloadMode, preamble,txvector);
  double duration5 = GetPayloadDurationMicroSeconds (size, txvector);
  double duration = duration1 + duration2 + duration3 + duration4 + duration5;
  NS_LOG_DEBUG("CalculateTxDuration" << duration1 << " "<< duration2 <<" "<< duration3 <<" "<< duration4 <<" "<< duration5 << " " <<duration);
...
WifiPhy::CalculatePlcpDuration (WifiTxVector txvector, WifiPreamble preamble)
{
  ...
  NS_LOG_DEBUG("CalculatePlcpDuration" << duration1 << " "<< duration2 <<" "<< duration3 <<" "<< duration4 <<" "<< duration);
...
void
WifiPhy::NotifyRxHeaderEnd (Ptr<const Packet> packet)
{
  m_phyRxHeaderEndTrace (packet);
}
