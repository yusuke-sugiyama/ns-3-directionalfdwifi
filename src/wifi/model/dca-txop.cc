...
{
  NS_LOG_FUNCTION (this << m_queue->IsEmpty () << m_dcf->IsAccessRequested ());
...
void
DcaTxop::NotifySecondaryTransmissionRequested ()
{
  NS_LOG_FUNCTION(this);

  if(IsCurrentPacket () || !IsQueueEmpty ())
    {
      m_dcf->NotifyAccessRequested ();
      if(IsSecondaryTransmissionGranted())
        {
          m_manager->CancelAccessRequested ();
          SecondaryTransmissionGranted();
        }
    }
}

bool
DcaTxop::IsSecondaryTransmissionGranted (void)
{
  NS_LOG_FUNCTION (m_manager->IsBusyForFullDuplex() << m_manager->GetSecondaryGrantedTime () << Simulator::Now());
  if (!m_manager->IsBusyForFullDuplex() &&
      m_manager->GetSecondaryGrantedTime () <= Simulator::Now())
    {
      return true;
    }
  return false;
...
bool
DcaTxop::IsSendBusytoneGranted (void)
{
  NS_LOG_FUNCTION (IsQueueEmpty () << IsCurrentPacket ());
  if (IsSecondaryTransmissionGranted () &&
      IsQueueEmpty () &&
      !IsCurrentPacket ()
      )
    {
      NS_LOG_DEBUG("IsSendBusytoneGranted true");
      return true;
    }
  NS_LOG_DEBUG("IsSendBusytoneGranted false");
  return false;
}


void
DcaTxop::SecondaryTransmissionGranted (void)
{
  NS_LOG_FUNCTION (this);
  if (m_currentPacket == 0)
    {
  ...
  if (m_currentHdr.GetAddr1 ().IsGroup ())
    {
      ...
      Low ()->StartSecondaryTransmission (m_currentPacket,
                                 &m_currentHdr,
                                 params,
      ...
      if (NeedFragmentation ())
        {
          ...
          Low ()->StartSecondaryTransmission (fragment, &hdr, params,
                                     m_transmissionListener);
        }
      else
        {
          ...
          params.DisableNextData ();
          Low ()->StartSecondaryTransmission (m_currentPacket, &m_currentHdr,
