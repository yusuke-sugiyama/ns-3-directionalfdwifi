...
// original
void
WifiMacHeader::SetMoreData (bool moreData)
{
  if(moreData)
    {
      m_ctrlMoreData = 1;
...
WifiMacHeader::SetType (enum WifiMacType type)
{
  switch (type)
    {
    case WIFI_MAC_CTL_BUSY:
      m_ctrlType = TYPE_CTL;
      m_ctrlSubtype = SUBTYPE_CTL_BUSY;
      break;
  ...
  if(m_ctrlType == TYPE_DATA)
    {
      m_addr4 = Mac48Address ("ff:ff:ff:ff:ff:ff");
    }
...
WifiMacHeader::GetType (void) const
{
  switch (m_ctrlType)
    ...
    case TYPE_CTL:
      switch (m_ctrlSubtype)
        {
        case SUBTYPE_CTL_BUSY:
          return WIFI_MAC_CTL_BUSY;
          break;
...
bool
WifiMacHeader::IsBusy (void) const
{
  return (GetType () == WIFI_MAC_CTL_BUSY);
}
bool
...
WifiMacHeader::Print (std::ostream &os) const
{
  ...
  switch (GetType ())
    ...
    case WIFI_MAC_DATA:
      ...
      if (!m_ctrlToDs && !m_ctrlFromDs)
        {
          os << "DA=" << m_addr1 << ", SA=" << m_addr2 << ", BSSID=" << m_addr3 << ", SecondaryAddress=" << m_addr4;
        }
