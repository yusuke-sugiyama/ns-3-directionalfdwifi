...
#include "wifi-cosine-antenna-model.h"
#include "ns3/orientation-model.h"


NS_LOG_COMPONENT_DEFINE ("WifiCosineAntennaModel");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (WifiCosineAntennaModel);


TypeId 
WifiCosineAntennaModel::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::WifiCosineAntennaModel")
    .SetParent<WifiAntennaModel> ()
    .AddConstructor<WifiCosineAntennaModel> ()
    .AddAttribute ("Beamwidth",
                   "The 3dB beamwidth (degrees)",
                   DoubleValue (60),
                   MakeDoubleAccessor (&WifiCosineAntennaModel::SetBeamwidth,
                                       &WifiCosineAntennaModel::GetBeamwidth),
                   MakeDoubleChecker<double> (0, 180))
    .AddAttribute ("MaxGain",
                   "The gain (dB) at the antenna boresight (the direction of maximum gain)",
                   DoubleValue (6.0),
                   MakeDoubleAccessor (&WifiCosineAntennaModel::m_maxGain),
...
void 
WifiCosineAntennaModel::SetBeamwidth (double beamwidthDegrees)
{ 
...
double
WifiCosineAntennaModel::GetBeamwidth () const
{
  return RadiansToDegrees (m_beamwidthRadians);
}

double 
WifiCosineAntennaModel::DoGetGainDb (Angles a)
{
